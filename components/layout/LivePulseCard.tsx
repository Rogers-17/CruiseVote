  ...
  return (
    <div
      className={`relative overflow-hidden rounded-[2.5rem] border p-8 transition-all duration-500 ${
        isPulsing
          ? 'scale-[1.02] border-[--color-brand-yellow]/40 bg-[--color-brand-yellow]/10'
          : 'border-white/10 bg-white/5'
      }`}
    >
      {/* Dynamic Background Glow */}
      <div
        className={`absolute -top-10 -right-10 h-40 w-40 bg-[--color-brand-yellow] blur-[80px] transition-opacity duration-700 ${
      ...
      {isPulsing && (
        <div className="absolute inset-0 animate-ping rounded-[2.5rem] border-2 border-[--color-brand-yellow] opacity-20" />
      )}
      ...
      <div className="relative z-10 flex items-center justify-between">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span
              className={`h-2 w-2 rounded-full ${isPulsing ? 'animate-pulse bg-[--color-brand-yellow]' : 'bg-green-500'}`}
            />
          ...
          <h2
            className={`text-6xl font-black tracking-tighter italic transition-colors duration-300 ${
              isPulsing ? 'text-white' : 'text-[--color-brand-yellow]'
            }`}
          >
        ...
        <div
          className={`flex h-20 w-20 items-center justify-center rounded-3xl border-2 transition-all duration-500 ${
            isPulsing
              ? 'rotate-12 border-[--color-brand-yellow] bg-[--color-brand-yellow]'
        ...
        >
          {isPulsing ? (
            ...
            <TrendingUp className="h-10 w-10 text-[--color-brand-yellow]" />
          )}
        </div>
      </div>

      {/* Decorative "Voltage" Bar at bottom */}
      <div className="absolute bottom-0 left-0 h-1 w-full bg-gradient-to-r from-transparent via-[--color-brand-yellow] to-transparent opacity-30" />
    </div>
  );
}
